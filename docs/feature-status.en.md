...
| Codex 画图接口逆向 | ✅  | 已支持，仅 `Plus` / `Team` / `Pro` 订阅可用，模型别名为 `codex-gpt-image-2.5`；如有需要可自行在其他场景映射回 `gpt-image-2.5`。Codex 工具模型默认走 `gpt-image-2.5-flare`，也可指定 `codex-gpt-image-2.5-sunburst`。这是 Codex 逆向链路，用于和官网画图区分，同一账号通常会同时支持官网和 Codex 两份生图额度。 |
