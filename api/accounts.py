...
def create_router() -> APIRouter:
    ...
    async def get_accounts(
        ...
        if sort_by in {"priority", "quota"}:
            reverse = order != "asc"
            items.sort(key=lambda a: int(a.get(sort_by) or 0), reverse=reverse)
        return {"items": items}
    ...
    async def update_account(body: AccountUpdateRequest, authorization: str | None = Header(default=None)):
        ...
        updates = {key: value for key, value in {"type": body.type, "status": body.status, "quota": body.quota, "priority": body.priority, "proxy": body.proxy}.items() if value is not None}
        if not updates:
            raise HTTPException(status_code=400, detail={"error": "还没有检测到改动，请修改后再保存"})
